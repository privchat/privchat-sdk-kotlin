...
actual fun parseServerUrl(url: String): ServerEndpoint? = null // iOS implementation pending

/**
 * iOS actual - 构建时 cinterop 链接 privchat-ffi.a（Rust static lib）。
 * 当前为占位实现，完整实现需：
 * 1. 在 Gradle 中配置 cargo 构建 aarch64-apple-ios / aarch64-apple-ios-sim
 * 2. cinterop.def 指向 UniFFI C ABI 头文件并链接 .a
 * 3. KMP 不产出 XCFramework，产物为 klib（详见 ARTIFACT_CONTRACT.md）
 */
actual class PrivchatClient actual constructor() {
    actual suspend fun connect(): Result<Unit> = Result.failure(NotImplementedError("iOS implementation pending"))
    actual suspend fun disconnect(): Result<Unit> = Result.failure(NotImplementedError("iOS implementation pending"))
    actual fun isConnected(): Boolean = throw NotImplementedError("iOS implementation pending")
    actual fun connectionState(): ConnectionState = throw NotImplementedError("iOS implementation pending")
    actual fun currentUserId(): ULong? = throw NotImplementedError("iOS implementation pending")
    actual suspend fun shutdown(): Result<Unit> = Result.failure(NotImplementedError("iOS implementation pending"))
    ...
    actual suspend fun authenticate(userId: ULong, token: String, deviceId: String): Result<Unit> =
        Result.failure(NotImplementedError("iOS implementation pending"))
    actual suspend fun logout(): Result<Unit> = Result.failure(NotImplementedError("iOS implementation pending"))
    ...
    actual suspend fun runBootstrapSync(): Result<Unit> = Result.failure(NotImplementedError("iOS implementation pending"))
    actual fun runBootstrapSyncInBackground() {}
    actual suspend fun isBootstrapCompleted(): Result<Boolean> = Result.failure(NotImplementedError("iOS implementation pending"))
    ...
    actual companion object {
        actual fun create(config: PrivchatConfig): Result<PrivchatClient> =
            Result.failure(NotImplementedError("iOS implementation pending - requires PrivchatFFI XCFramework + cinterop"))
    }
}
