...
actual fun parseServerUrl(url: String): ServerEndpoint? = null // Linux implementation pending

/**
 * Linux Desktop actual - Trixnity cinterop 链接 privchat-ffi.so。
 * 当前为占位。
 */
actual class PrivchatClient actual constructor() {
    actual suspend fun connect(): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun disconnect(): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual fun isConnected(): Boolean = throw NotImplementedError("Linux implementation pending")
    actual fun connectionState(): ConnectionState = throw NotImplementedError("Linux implementation pending")
    actual fun currentUserId(): ULong? = throw NotImplementedError("Linux implementation pending")
    actual suspend fun shutdown(): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))

    actual suspend fun register(username: String, password: String, deviceId: String): Result<AuthResult> =
        Result.failure(NotImplementedError("Linux implementation pending"))
    ...
    actual suspend fun authenticate(userId: ULong, token: String, deviceId: String): Result<Unit> =
        Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun logout(): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    ...
    actual suspend fun sendMedia(channelId: ULong, filePath: String, options: SendMessageOptions?): Result<Pair<ULong, AttachmentInfo>> =
        Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun retryMessage(messageId: ULong): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun markAsRead(channelId: ULong, messageId: ULong): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun markFullyReadAt(channelId: ULong, messageId: ULong): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun revokeMessage(messageId: ULong): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun editMessage(messageId: ULong, newContent: String): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun addReaction(messageId: ULong, emoji: String): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun removeReaction(messageId: ULong, emoji: String): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    ...
    actual suspend fun getMessages(channelId: ULong, limit: UInt, beforeSeq: ULong?): Result<List<MessageEntry>> =
        Result.failure(NotImplementedError("Linux implementation pending"))
    actual suspend fun getMessageById(messageId: ULong): Result<MessageEntry?> = Result.failure(NotImplementedError("Linux implementation pending"))
    ...
    actual suspend fun runBootstrapSync(): Result<Unit> = Result.failure(NotImplementedError("Linux implementation pending"))
    actual fun runBootstrapSyncInBackground() {}
    actual suspend fun isBootstrapCompleted(): Result<Boolean> = Result.failure(NotImplementedError("Linux implementation pending"))
