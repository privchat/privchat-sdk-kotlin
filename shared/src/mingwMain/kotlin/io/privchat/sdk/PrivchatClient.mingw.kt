...
actual fun parseServerUrl(url: String): ServerEndpoint? = null // mingw implementation pending

/**
 * Windows Desktop actual（mingwX64）- cinterop 链接 privchat-ffi.dll。
 * 纯 Kotlin/Native，不依赖 JVM。当前为占位。
 */
actual class PrivchatClient actual constructor() {
    actual suspend fun connect(): Result<Unit> = Result.failure(NotImplementedError("mingw implementation pending"))
    actual suspend fun disconnect(): Result<Unit> = Result.failure(NotImplementedError("mingw implementation pending"))
    actual fun isConnected(): Boolean = throw NotImplementedError("mingw implementation pending")
    actual fun connectionState(): ConnectionState = throw NotImplementedError("mingw implementation pending")
    actual fun currentUserId(): ULong? = throw NotImplementedError("mingw implementation pending")
    actual suspend fun shutdown(): Result<Unit> = Result.failure(NotImplementedError("mingw implementation pending"))

    actual suspend fun register(username: String, password: String, deviceId: String): Result<AuthResult> =
        Result.failure(NotImplementedError("mingw implementation pending"))
    ...
    actual suspend fun authenticate(userId: ULong, token: String, deviceId: String): Result<Unit> =
        Result.failure(NotImplementedError("mingw implementation pending"))
    actual suspend fun logout(): Result<Unit> = Result.failure(NotImplementedError("mingw implementation pending"))
    ...
    actual suspend fun sendMedia(channelId: ULong, filePath: String, options: SendMessageOptions?): Result<Pair<ULong, AttachmentInfo>> =
        Result.failure(NotImplementedError("mingw implementation pending"))
    actual suspend fun retryMessage(messageId: ULong): Result<Unit> = Result.failure(NotImplementedError("mingw implementation pending"))
    ...
    actual suspend fun runBootstrapSync(): Result<Unit> = Result.failure(NotImplementedError("mingw implementation pending"))
